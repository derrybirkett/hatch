...
export async function initCommand(
  ...
  try {
    ...
    console.log(
      chalk.gray('Bootstrap production-ready SaaS applications in minutes.\n')
    );
    ...
    if (projectName && !/^[a-z0-9-]+$/.test(projectName)) {
      ...
      console.error(
        chalk.gray('Example: my-saas-app, fitness-tracker, analytics-dashboard')
      );
      process.exit(1);
    }
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('already exists')) {
          ...
          console.error(
            chalk.gray('https://github.com/yourusername/hatch/issues')
          );
        }
      } else {
    ...
    console.log(
      chalk.gray('    ├── apps/           (website, dashboard, api)')
    );
    ...
    console.log(chalk.gray('   • Dashboard: http://localhost:4200'));
    console.log(chalk.gray('   • Website:   http://localhost:4201'));
    console.log(chalk.gray('   • API:       http://localhost:4202\n'));
