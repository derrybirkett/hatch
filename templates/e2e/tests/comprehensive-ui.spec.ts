...
async function captureFatalErrors(page: Page): Promise<string[]> {
  ...
  page.on('console', msg => {
    if (msg.type() === 'error') {
      ...
      const isFatalError = text.includes('is not defined') ||
...
async function verifyPageLoadsCleanly(page: Page, url: string, expectedElements: string[]) {
...
test.describe('CRITICAL PATH 1: Signup Page', () => {
  ...
  test('empty form submission is handled without crashes', async ({ page }) => {
    ...
    if (errors.length > 0) {
      throw new Error(`Empty form submission caused errors: ${errors.join(', ')}`);
    }
  ...
  test('filled form submission works without JavaScript errors', async ({ page }) => {
    const errors = await captureFatalErrors(page);
    ...
    if (errors.length > 0) {
      throw new Error(`Form submission caused JavaScript errors: ${errors.join(', ')}`);
    }
  ...
  test('sign in link navigates to login page', async ({ page }) => {
    ...
    const signInLink = page.locator('a:has-text("Sign in"), a:has-text("Log in"), a[href*="login"]').first();
...
test.describe('CRITICAL PATH 2: Login Page', () => {
  ...
  test('filled form submission works without JavaScript errors', async ({ page }) => {
    ...
    if (errors.length > 0) {
      throw new Error(`Login form submission caused JavaScript errors: ${errors.join(', ')}`);
    }
  ...
  test('sign up link navigates to signup page', async ({ page }) => {
    ...
    const signUpLink = page.locator('a:has-text("Sign up"), a:has-text("Create account"), a[href*="signup"]').first();
...
test.describe('CRITICAL PATH 3: Dashboard Page', () => {
  test('page loads without JavaScript errors', async ({ page }) => {
    ...
    if (errors.length > 0) {
      throw new Error(`Dashboard page has console errors: ${errors.join(', ')}`);
    }
  ...
  test('dashboard is accessible (auth implementation pending)', async ({ page }) => {
    await page.goto('http://localhost:3000/dashboard');
    await page.waitForLoadState('networkidle');
...
test.describe('CRITICAL PATH 4: Direct URL Navigation', () => {
  ...
  for (const route of routes) {
    test(`${route} loads without JavaScript errors`, async ({ page }) => {
      ...
      if (errors.length > 0) {
        throw new Error(`Route ${route} has console errors: ${errors.join(', ')}`);
      }
